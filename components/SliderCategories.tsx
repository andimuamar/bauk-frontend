    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Slide {currentIndex + 1} dari {maxIndex + 1} · {visibleCards} kartu /
          tampilan
        </p>
      </div>
          ...
          {items.map((item: any) => {
            ...
            return (
              <div
                key={item.number}
                className="flex-none"
                style={{ width: `calc(${100 / visibleCards}% - 1.25rem)` }}
              >
                <div className="group relative h-full rounded-3xl border border-slate-200 bg-white p-6 pt-10 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900">
